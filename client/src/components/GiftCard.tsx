...
export default function GiftCard({ 
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full p-4 sm:p-5 md:p-7 flex flex-col justify-between text-white">
        {/* Top section */}
        <div className="flex items-start justify-between gap-2">
          <div className="text-7xl sm:text-8xl md:text-9xl leading-none" data-testid="gift-emoji">{emoji}</div>
          {recipientName && (
            <div className="text-right">
              <div className="text-xs sm:text-sm md:text-base opacity-90 leading-tight">Gift Card for</div>
              <div className="text-sm sm:text-base md:text-lg font-semibold uppercase leading-tight" data-testid="text-recipient-name">{recipientName}</div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-end justify-between gap-2">
          {message && (
            <div className="flex-1 flex items-end mr-2">
              <p className="text-base sm:text-lg md:text-2xl font-script italic opacity-95 line-clamp-2" data-testid="text-message">
                "{message}"
              </p>
            </div>
          )}
          <div className="text-right flex items-end shrink-0">
            <div className="text-4xl sm:text-6xl md:text-8xl font-bold leading-none" data-testid="text-amount">
              ${amount}
            </div>
          </div>
        </div>
